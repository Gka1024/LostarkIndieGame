...
이 프로젝트는 단순히 하나의 게임을 완성하는 것이 아니라,  
**전투 시스템을 설계하고 확장해 나가는 개발자로서의 성장을 기록하는 프로젝트**입니다.

주요 목표는 다음과 같습니다:

✔ 구조적으로 확장 가능한 전투 아키텍처를 직접 설계하고 개선하는 경험  
✔ AI / Buff / Turn Flow 등 복잡한 시스템을 분리하고 연결하는 능력 강화  
✔ 기능 구현이 아닌 **설계 의도와 책임 분리 중심의 코드 작성 습관** 체득  
✔ 실전 게임 구조를 스스로 설계 → 구현 → 리팩토링하는 반복 학습
